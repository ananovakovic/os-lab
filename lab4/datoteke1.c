...
int procitaj(int id, void *p, size_t koliko)
{
	...
	int blok_dat = dat_kazaljka / BLOCK_SIZE;
	...
	while (jos > 0 //još se može čitati
		...
		&& blokovi_datoteke[blok_dat] >= 0) //postoji taj blok datoteke
	{
	*buffer = datotecna_tablica[id].blokovi[blok_datoteke];
	// dohvati blok diska blokovi_datoteke[blok_dat] u buffer
	kopirati = min(jos, vel_datoteke - dat_kazaljka,
			BLOCK_SIZE - dat_kazaljka % BLOCK_SIZE)
	p = *buffer + dat_kazaljka % BLOCK_SIZE;
...
int zapisi(int id, void *p, size_t koliko)
{
	...
	{
		if (blokovi_datoteke[blok_dat] == -1) 
		{
		//nađi slobodni blok i ažuriraj blokovi_datoteke
		datotecna_tablica[id].blokovi[blok_datoteke] = *buffer;
		}
		else exit(1);
	*buffer = blokovi_datoteke[blok_dat];
	//dohvati blok diska blokovi_datoteke[blok_dat] u buffer
	
	kopirati = min(jos, BLOCK_SIZE - dat_kazaljka % BLOCK_SIZE);
	//kopiraj iz p u buffer + dat_kazaljka % BLOCK_SIZE
	*buffer = *p dat_kazaljka % BLOCK_SIZE;
	//zapiši blok na disk (buffer => blokovi_datoteke[blok_dat])
	blokovi_datoteke[blok_dat] = *buffer;
	p += kopirati;
	dat_kazaljka += kopirati;
	jos -= kopirati;
	blok_dat++;
}
	vel_datoteke = max(dat_kazaljka, vel_datoteke-kopirati);
	return koliko - jos;
